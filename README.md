This is a Modded Telegram Bot written in Python for mirroring files on the Internet to your Google Drive or Telegram. Mod by Sanwar-Hosen.

# Features:

## By [TheSano](https://github.com/sanwar-hosen)

### Dynamic Pictures
- Shows Pictures in status, stats and start message.
Picture can be uploaded by own in PIC config, or can be auto downloaded from wallcraft through catagory selection.
(From [Weebzone](https://github.com/weebzone/WZML))
...
- `STATUS_LIMIT`: Limit the no. of tasks shown in status message with buttons. **NOTE**: Recommended limit is `2` tasks If Dynamic image is enabled. `Int`
...
### PICTURES
- `START_PIC`: Bot Start message Pic Url.
- `PICS`: bot status, stats message pics, separate with space if multiple pics.
